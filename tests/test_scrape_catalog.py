...
def test_parse_delivery_info():
    ...
    # Test with status
    delivery_type, delivery_torres, project_status = parse_delivery_info("MAYO 2026 - En construcción")
    assert "MAYO 2026" in delivery_type
    assert project_status == "En Construcción"
    
    # Test with towers
    delivery_type, delivery_torres, project_status = parse_delivery_info("TORRE A INMEDIATA, TORRE B MAYO 2026")
...
async def test_wait_for_more_projects_success():
    ...
    result = await wait_for_more_projects(
        page,
        "a[href*='/proyecto/']",
        ["url1", "url2"],
        "tr",
...
def test_setup_db():
    ...
    try:
        ...
        expected_columns = {
            'id', 'name', 'zone', 'delivery_type', 'delivery_torres',
            'project_status', 'price_from', 'developer', 'commission',
            'has_ley_vp', 'location', 'image_url', 'detail_url', 'scraped_at'
        }
...
async def test_extract_delivery_and_status_from_column_full():
    ...
    delivery_type, delivery_torres, project_status = await extract_delivery_and_status_from_column(col)
...
async def test_extract_delivery_and_status_from_column_none():
    ...
    delivery_type, delivery_torres, project_status = await extract_delivery_and_status_from_column(None)
...
def test_parse_delivery_info_en_construccion():
    ...
    delivery_type, delivery_torres, project_status = parse_delivery_info("Junio 2026 - En construcción")
            ...
            with patch('scrape_catalog_phase1.scroll_container_to_bottom', return_value=True):
                with patch('scrape_catalog_phase1.scroll_last_row_into_view', return_value=True):
                    with patch('scrape_catalog_phase1.scroll_catalog', return_value=None):
                        with patch('scrape_catalog_phase1.wait_for_more_projects', return_value=True):
    ...
    with patch('scrape_catalog_phase1.pick_selector', return_value=("button", 1)):
        with patch('scrape_catalog_phase1.get_project_hrefs', return_value=["/proyecto/1"]):
            with patch('scrape_catalog_phase1.scroll_container_to_bottom', return_value=True):
                with patch('scrape_catalog_phase1.scroll_last_row_into_view', return_value=True):
                    with patch('scrape_catalog_phase1.scroll_catalog', return_value=None):
                        with patch('scrape_catalog_phase1.wait_for_more_projects', return_value=False):
                            result = await click_load_more(page, "a[href*='/proyecto/']", "tr")
